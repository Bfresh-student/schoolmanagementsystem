...
const API_CONFIG = {
  BASE_URL:
    "https://schoolmanagementsystem-production-6624.up.railway.app/api/v1",
  TOKEN_KEY: "authToken",
...
async function apiClientRequest(
  path,
  { method = "GET", body = null, retry = true } = {},
) {
  ...
  if (response.status === 401 && retry) {
    ...
    if (refreshed)
      return apiClientRequest(path, { method, body, retry: false });
    redirectToLogin();
...
const AuthAPI = {
  async login(email, password) {
    const data = await apiClientRequest("/auth/users/login/", {
      method: "POST",
      body: { email, password },
    });
  ...
  async updateMe(body) {
    return apiClientRequest("/auth/users/me/update/", {
      method: "PATCH",
      body,
    });
  },
  async changePassword(body) {
    return apiClientRequest("/auth/users/me/change-password/", {
      method: "POST",
      body,
    });
  },
...
const SettingsAPI = {
  get() {
    return apiClientRequest("/auth/users/settings/");
  },
  save(settings) {
    return apiClientRequest("/auth/users/settings/", {
      method: "PATCH",
...
const AuditAPI = {
  list() {
    return apiClientRequest("/hr/audit-log/?page_size=200");
  },
};
...
const GlobalSearchAPI = {
  search(query) {
    return apiClientRequest(
      `/auth/users/global-search/?q=${encodeURIComponent(query)}`,
    );
  },
};
...
const StudentsAPI = {
  ...
  async list(params = {}) {
    ...
    const data = await apiClientRequest(
      `/students/students/?${searchParams.toString()}`,
    );
    return data.results || data;
  },
  async search(query) {
    const data = await apiClientRequest(
      `/students/students/?search=${encodeURIComponent(query)}&page_size=1000`,
    );
    return data.results || data;
  },
};
...
const ClassesAPI = {
  async list(params = {}) {
    ...
    const data = await apiClientRequest(
      `/students/classes/?${searchParams.toString()}`,
    );
    return data.results || data;
  },
  async update(id, payload) {
    return apiClientRequest("/students/classes/" + id + "/", {
      method: "PATCH",
      body: payload,
    });
  },
...
const AcademicYearsAPI = {
  ...
  async create(body) {
    return apiClientRequest("/students/academic-years/", {
      method: "POST",
      body,
    });
  },
  async update(id, body) {
    return apiClientRequest(`/students/academic-years/${id}/`, {
      method: "PATCH",
      body,
    });
  },
...
const AssessmentsAPI = {
  async list({ schoolClass, academicYear } = {}) {
    ...
    const data = await apiClientRequest(
      `/grades/assessments/${params.toString() ? `?${params}` : ""}`,
    );
    return data.results || data;
  },
  async create(payload) {
    return apiClientRequest("/grades/assessments/", {
      method: "POST",
      body: payload,
    });
  },
  async update(id, payload) {
    return apiClientRequest(`/grades/assessments/${id}/`, {
      method: "PATCH",
      body: payload,
    });
  },
...
async function apiFetchMultipart(path, formData, method = "POST") {
  ...
  if (response.status === 401) {
    ...
    if (await tryRefreshToken())
      return apiFetchMultipart(path, formData, method);
    redirectToLogin();
    throw new Error("UNAUTHORIZED");
  }
  if (!response.ok) {
...
const HRAPI = {
  ...
  teachers() {
    return apiClientRequest("/teachers/?page_size=1000").then(
  ...
  employees(params = {}) {
    return this.list("employees", params);
  },
  searchEmployees(query) {
    return apiClientRequest(
      `/hr/employees/?search=${encodeURIComponent(query)}&page_size=1000`,
    ).then((data) => data.results || data);
  },
  createEmployee(body) {
    return apiClientRequest("/hr/employees/", { method: "POST", body });
  },
  updateEmployee(id, body) {
    return apiClientRequest(`/hr/employees/${id}/`, { method: "PATCH", body });
  },
  deleteEmployee(id) {
    return apiClientRequest(`/hr/employees/${id}/`, { method: "DELETE" });
  },

  // --- Présences du personnel ---
  attendances(todayOnly = true) {
    const suffix = todayOnly
      ? `?date=${new Date().toISOString().slice(0, 10)}&page_size=1000`
      : "?page_size=1000";
    return apiClientRequest(`/hr/attendances/${suffix}`).then(
      (data) => data.results || data,
    );
  },
  attendanceReport(start, end) {
    return apiClientRequest(
      `/hr/attendances/report-summary/?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`,
    );
  },
  createAttendance(body) {
    return apiClientRequest("/hr/attendances/", { method: "POST", body });
  },
  updateAttendance(id, body) {
    return apiClientRequest(`/hr/attendances/${id}/`, {
      method: "PATCH",
      body,
    });
  },
  deleteAttendance(id) {
    return apiClientRequest(`/hr/attendances/${id}/`, { method: "DELETE" });
  },

  // --- Recrutement ---
  candidates() {
    return this.list("candidates");
  },
  createCandidate(formData) {
    return apiFetchMultipart("/hr/candidates/", formData);
  },
  updateCandidate(id, formData) {
    return apiFetchMultipart(`/hr/candidates/${id}/`, formData, "PATCH");
  },
  candidateDocuments(candidateId) {
    return apiClientRequest(
      `/hr/candidate-documents/?candidate=${encodeURIComponent(candidateId)}`,
    );
  },
  createCandidateDocument(formData) {
    return apiFetchMultipart("/hr/candidate-documents/", formData);
  },
  deleteCandidateDocument(id) {
    return apiClientRequest(`/hr/candidate-documents/${id}/`, {
      method: "DELETE",
    });
  },
  deleteCandidate(id) {
    return apiClientRequest(`/hr/candidates/${id}/`, { method: "DELETE" });
  },
  scheduleInterview(id, body) {
    return apiClientRequest(`/hr/candidates/${id}/schedule_interview/`, {
      method: "POST",
      body,
    });
  },
  hireCandidate(id, body = {}) {
    return apiClientRequest(`/hr/candidates/${id}/hire/`, {
      method: "POST",
      body,
    });
  },
  rejectCandidate(id) {
    return apiClientRequest(`/hr/candidates/${id}/reject_after_interview/`, {
      method: "POST",
    });
  },
  // --- Contrats ---
  contracts(params = {}) {
    return this.list("contracts", params);
  },
  createContract(body) {
    return apiClientRequest("/hr/contracts/", { method: "POST", body });
  },
  updateContract(id, body) {
    return apiClientRequest(`/hr/contracts/${id}/`, { method: "PATCH", body });
  },
  deleteContract(id) {
    return apiClientRequest(`/hr/contracts/${id}/`, { method: "DELETE" });
  },
  terminateContract(id, body) {
    return apiClientRequest(`/hr/contracts/${id}/terminate/`, {
      method: "POST",
      body,
    });
  },
  ...
  // --- Salaires ---
  salaries(params = {}) {
    return this.list("salaries", params);
  },
  createSalary(body) {
    return apiClientRequest("/hr/salaries/", { method: "POST", body });
  },
  updateSalary(id, body) {
    return apiClientRequest(`/hr/salaries/${id}/`, { method: "PATCH", body });
  },
  deleteSalary(id) {
    return apiClientRequest(`/hr/salaries/${id}/`, { method: "DELETE" });
  },
  markSalaryPaid(id, body) {
    return apiClientRequest(`/hr/salaries/${id}/mark_paid/`, {
      method: "POST",
      body,
    });
  },
  salaryBalance(teacherId) {
    return apiClientRequest(
      `/hr/salaries/?teacher=${teacherId}&page_size=100`,
    ).then((d) => d.results || d);
  },

  // --- Congés ---
  leaves(params = {}) {
    return this.list("leaves", params);
  },
  leaveTypes() {
    return this.list("leave-types");
  },
  createLeave(body) {
    return apiClientRequest("/hr/leaves/", { method: "POST", body });
  },
  updateLeave(id, body) {
    return apiClientRequest(`/hr/leaves/${id}/`, { method: "PATCH", body });
  },
  deleteLeave(id) {
    return apiClientRequest(`/hr/leaves/${id}/`, { method: "DELETE" });
  },
  approveLeave(id) {
    return apiClientRequest(`/hr/leaves/${id}/approve/`, { method: "POST" });
  },
  rejectLeave(id) {
    return apiClientRequest(`/hr/leaves/${id}/reject/`, { method: "POST" });
  },
  leaveBalance() {
    return apiClientRequest("/hr/leaves/balance/");
  },

  // --- Évaluations ---
  evaluations(params = {}) {
    return this.list("evaluations", params);
  },
  createEvaluation(body) {
    return apiClientRequest("/hr/evaluations/", { method: "POST", body });
  },
  updateEvaluation(id, body) {
    return apiClientRequest(`/hr/evaluations/${id}/`, {
      method: "PATCH",
      body,
    });
  },
  deleteEvaluation(id) {
    return apiClientRequest(`/hr/evaluations/${id}/`, { method: "DELETE" });
  },
  acknowledgeEvaluation(id, comments) {
    return apiClientRequest(`/hr/evaluations/${id}/acknowledge/`, {
      method: "POST",
      body: { comments },
    });
  },
  ...
  createDocument(formData) {
    return apiFetchMultipart("/hr/documents/", formData);
  },
  updateDocument(id, formData) {
    return apiFetchMultipart(`/hr/documents/${id}/`, formData, "PATCH");
  },
  deleteDocument(id) {
    return apiClientRequest(`/hr/documents/${id}/`, { method: "DELETE" });
  },
  getDocument(id) {
    return apiClientRequest(`/hr/documents/${id}/`);
  },

  // --- Audit Log ---
  auditLog() {
    return this.list("audit-log");
  },
};
window.HRAPI = HRAPI;
...
const InscriptionsAPI = {
  ...
  async search(query) {
    const data = await apiClientRequest(
      `/enrollments/inscriptions/?search=${encodeURIComponent(query)}&page_size=1000`,
    );
    return data.results || data;
  },
  async create({
    ...
    if (local_uuid) body.local_uuid = local_uuid;
    return apiClientRequest("/enrollments/inscriptions/", {
      method: "POST",
      body,
    });
  },
  ...
  async approve(id) {
    return apiClientRequest(`/enrollments/inscriptions/${id}/approve/`, {
      method: "POST",
    });
  },
...
const OfflineQueue = {
  read() {
    try {
      return (
        JSON.parse(localStorage.getItem(API_CONFIG.OFFLINE_QUEUE_KEY)) || []
      );
    } catch (_) {
      return [];
    }
  ...
  async _resolveStudentAccount(item) {
    ...
    if (!item.first_name || !item.last_name) {
      console.warn(
        "Item hors-ligne incomplet (nom manquant), impossible de créer le compte étudiant :",
        item.local_uuid,
      );
      return null;
    }
    ...
    try {
      ...
      console.warn(
        "Échec de création du compte étudiant pour",
        item.local_uuid,
        err,
      );
      return null;
  ...
  async sync() {
    ...
    try {
      ...
      const succeededUuids = (res.results || [])
        ...
        .map((r) => r.local_uuid);
      const stillFailed = resolved.filter(
        (i) => !succeededUuids.includes(i.local_uuid),
      );
      OfflineQueue.write([...stillPending, ...stillFailed]);
      return {
        synced: succeededUuids.length,
...
window.addEventListener("online", () => {
  OfflineQueue.sync().then((res) => {
    if (res.synced > 0 && window.showToast) {
      showToast(
        `${res.synced} inscription(s) hors-ligne synchronisée(s)`,
        "success",
      );
    }
  });
