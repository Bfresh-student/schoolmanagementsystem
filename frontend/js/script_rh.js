...
let defaultEmployees = [
  {
    ...
    prime: 15000,
    cours: [],
    diplomes: ["Licence en Gestion", "Master en Administration"],
  },
  {
    id: "EMP-2024-002",
    prenom: "Marie",
    ...
    sexe: "Femme",
    tel: "+509 33 22 11 00",
    email: "marie@cejec.edu.ht",
    fonction: "Secrétaire",
    dept: "Administration",
    embauche: "2021-03-10",
    statut: "Actif",
    adresse: "Pétion-Ville",
    salaire: 65000,
    prime: 8000,
    cours: [],
    diplomes: ["BTS Secrétariat"],
  },
  {
    id: "EMP-2024-003",
    prenom: "Carline",
  ...
  },
  {
    ...
    sexe: "Homme",
    tel: "+509 40 11 22 33",
    email: "marc.arthur@cejec.edu.ht",
    fonction: "Directeur",
    dept: "Direction Générale",
    embauche: "2018-01-01",
    statut: "Actif",
    ...
    prime: 25000,
    cours: [],
    diplomes: ["Doctorat en Éducation"],
  },
  {
    id: "EMP-2024-008",
    prenom: "Sophie",
  ...
  },
  {
    ...
    prime: 5000,
    cours: [],
    diplomes: ["Licence Bibliothéconomie"],
  },
  {
    id: "EMP-2024-010",
    prenom: "Nathalie",
    ...
    sexe: "Femme",
    tel: "+509 43 44 55 66",
    email: "nathalie@cejec.edu.ht",
    fonction: "Agent Communication",
    dept: "Communication",
    embauche: "2023-01-10",
    statut: "Actif",
    adresse: "Pétion-Ville",
    ...
    prime: 7000,
    cours: [],
    diplomes: ["Licence Communication"],
  },
  {
    id: "EMP-2024-011",
    prenom: "André",
...
let editingId = null,
  ...
  selectedFicheEmp = null,
  selectedCoursProfId = null,
  editingCandidatIndex = null;
let editingEvalIndex = null;
let selectedProfilIndex = null;
let pdfDoc = null,
  pdfCurrentPage = 1,
  pdfTotalPages = 0;
const matriculePrefix = "em";
...
function getProfs() {
  return employees.filter((e) => e.fonction === "Professeur");
}
...
function getDocColor(nom) {
  const ext = nom.split(".").pop().toLowerCase();
  if (ext === "pdf") return "var(--red)";
  if (ext === "doc" || ext === "docx") return "#3b82f6";
  if (ext === "jpg" || ext === "jpeg" || ext === "png") return "var(--purple)";
  if (ext === "xls" || ext === "xlsx") return "var(--success)";
  return "var(--muted)";
}
function getDocTypeClass(type) {
  if (type === "CV") return "pill-success";
  if (type === "Contrat") return "pill-info";
  if (type === "Diplôme") return "pill-purple";
  if (type === "Pièce identité") return "pill-warning";
  return "pill-muted";
}
...
function findEmployeeByDisplayName(name) {
  ...
  return employees.find(
    (e) => `${e.prenom} ${e.nom}`.trim().toLowerCase() === target,
  );
}
...
function mapCandidateFromAPI(c) {
  return {
    ...
    dateCandidature: c.application_date
      ? new Date(c.application_date).toLocaleDateString("fr-FR")
      : new Date().toLocaleDateString("fr-FR"),
    dateEntretien: c.interview_date
      ? new Date(c.interview_date).toLocaleDateString("fr-FR")
      : "",
    heureEntretien: c.interview_time
      ? String(c.interview_time).slice(0, 5)
      : "",
...
function handleCandidateDocuments(input) {
  ...
  if (display)
    display.textContent = files.length
      ? `${files.length} pièce(s) sélectionnée(s)`
      : "";
}
...
async function generatePreviewPDF(docName) {
  try {
    ...
    pdf.setFontSize(11);
    pdf.text(
      "Centre d'Etudes des Jeunes en Entrepreneuriat et Commerce",
      105,
      65,
      { align: "center" },
    );
    ...
    pdf.text(
      "Document faisant partie de la bibliotheque RH du CEJEC.",
      105,
      90,
      { align: "center" },
    );
    pdf.text(`Date: ${new Date().toLocaleDateString("fr-FR")}`, 105, 100, {
      align: "center",
    });
...
function switchPreviewTab(evt, tab, docName) {
  document
    .querySelectorAll(".pdf-preview-tab")
...
function voirDoc(nom) {
  ...
  if (isCandidat && candidatName) {
    const candidat = candidatsData.find(
      (c) => c.prenom + " " + c.nom === candidatName,
    );
    if (candidat) {
  ...
  document.getElementById("viewDocTitle").innerHTML =
    `<i class="fas fa-file-alt"></i> ${doc.nom}`;
  document.getElementById("viewDocContent").innerHTML = detailsHTML;
  document.getElementById("viewDocDownloadBtn").onclick = function () {
    telechargerDoc(doc.nom);
  };
...
function telechargerDoc(nom) {
  ...
  if (doc) {
    ...
    setTimeout(
      () => showToast(`✅ "${doc.nom}" téléchargé avec succès`, "success"),
      1000,
    );
  }
}
...
async function uploadDocument() {
  ...
  showToast(
    "⚠️ Sélectionnez un employé synchronisé et un fichier à téléverser",
    "error",
  );
}
...
function renderPage(page) {
  ...
  switch (page) {
    case "employes":
      mc.innerHTML = renderEmployes();
      break;
    case "professeurs":
      loadAndRenderProfesseurs();
      break;
    case "presences":
      loadAndRenderPresences();
      break;
    case "conges":
      loadAndRenderConges();
      break;
    case "salaires":
      loadAndRenderSalaires();
      break;
    case "fichepaie":
      mc.innerHTML = renderFichePaie();
      break;
    case "recrutement":
      loadAndRenderRecrutement();
      break;
    case "documents":
  ...
  employeeSearchTimer = setTimeout(async () => {
    try {
      ...
      const allowedIds = new Set(
        remoteEmployees.map((item) => String(item.id)),
      );
      if (tbody)
        tbody.querySelectorAll("tr").forEach((tr, index) => {
          const employee = employees[index];
          tr.style.display =
            employee?._hrEmployeeId &&
            allowedIds.has(String(employee._hrEmployeeId))
              ? ""
              : "none";
        });
    } catch (error) {
...
function viewProfil(i) {
  ...
  document.getElementById("profilName").innerHTML =
    `<i class="fas fa-id-card"></i> ${e.prenom} ${e.nom}`;
...
function editEmp(i) {
  editingId = i;
  const e = employees[i];
  document.getElementById("modalEmpTitle").innerHTML =
    `<i class="fas fa-edit"></i> Modifier ${e.prenom} ${e.nom}`;
...
async function saveEmployeeLocal() {
  ...
  if (!prenom || !nom) {
    showToast("⚠️ Prénom et nom obligatoires", "error");
    return;
  }
  const payload = {
    prenom,
    ...
    embauche:
      document.getElementById("empEmbauche").value ||
  ...
  try {
    ...
    if (localIdx !== -1) localEmployees[localIdx] = saved;
    else localEmployees.push(saved);
    localStorage.setItem("cejec_employees_rh", JSON.stringify(localEmployees));
  } catch (apiErr) {
...
function openCoursModal(profId) {
  ...
  document.getElementById("addCoursTitle").innerHTML =
    `<i class="fas fa-book"></i> Cours - ${emp.prenom} ${emp.nom}`;
...
function refreshCoursList() {
  ...
  if (emp.cours && emp.cours.length > 0) {
    container.innerHTML = emp.cours
      .map(
        (c, i) =>
          `<span class="pill pill-info" style="display:inline-flex;align-items:center;gap:5px">${c}<span class="badge-remove" onclick="retirerCours('${selectedCoursProfId}',${i})"><i class="fas fa-times"></i></span></span>`,
      )
      .join("");
  } else {
    container.innerHTML =
      '<span style="color:var(--muted)">Aucun cours assigné</span>';
  }
}
...
function ajouterCoursProf() {
  ...
  if (!nouveauCours) {
    showToast("⚠️ Entrez un nom de cours", "error");
    return;
  }
  const emp = employees.find((e) => e.id === profId);
  if (!emp) return;
  if (!emp.cours) emp.cours = [];
  if (emp.cours.includes(nouveauCours)) {
    showToast("⚠️ Ce cours est déjà assigné", "info");
    return;
  }
...
function voirEmploiTemps(id) {
  ...
  if (emp) {
    document.getElementById("emploiTempsTitle").innerHTML =
      `<i class="fas fa-calendar-alt"></i> Emploi du Temps - ${emp.prenom} ${emp.nom}`;
    ...
    heures.forEach((h) => {
      ...
      jours.forEach(() => {
        const coursDispo = emp.cours?.length
          ? emp.cours[Math.floor(Math.random() * emp.cours.length)]
          : "—";
...
function renderPresences() {
  ...
  const employesAujourdhui = employees.filter(
    (e) => e.statut !== "Suspendu" && e.statut !== "Terminé",
  );
  ...
  const presentsCount = rowsData.filter(
    (r) =>
      r.isPointed &&
      (r.pres.statut === "Présent" || r.pres.statut === "Retard"),
  ).length;
  const retardsCount = rowsData.filter(
    (r) => r.isPointed && r.pres.statut === "Retard",
  ).length;
  const absentsCount = rowsData.filter(
    (r) => r.isPointed && r.pres.statut === "Absent",
  ).length;
  const tauxPresence =
    totalEmployees > 0 ? Math.round((presentsCount / totalEmployees) * 100) : 0;
  ...
  let rows = displayedRows
    .map(({ employee: e, empIndex, pres, isPointed }) => {
      const isRecruit =
        e._source === "Recrutement" ||
        String(e.id).startsWith("REC-") ||
        (candidatsData &&
          candidatsData.some(
            (c) =>
      ...
      const recruitBadge = isRecruit
        ? `<span class="pill pill-info" style="font-size:0.7rem;padding:2px 6px;margin-left:6px" title="Enregistré via le recrutement"><i class="fas fa-briefcase"></i> Recruté</span>`
        : "";
...
function pointerPresenceModal(empIndex) {
  ...
  const today = getTodayDateString();
  document.getElementById("pointageTitle").innerHTML =
    `<i class="fas fa-clock"></i> Pointer - ${e.prenom} ${e.nom}`;
...
function corrigerHeureModal(empIndex) {
  ...
  const today = getTodayDateString();
  const nomComplet = `${e.prenom} ${e.nom}`.trim().toLowerCase();
  const existing = presencesData.find(
    (p) =>
      p.date === today &&
  ...
  document.getElementById("pointageTitle").innerHTML =
    `<i class="fas fa-edit"></i> Corriger - ${e.prenom} ${e.nom}`;
  document.getElementById("pointageEmpIndex").value = empIndex;
  document.getElementById("pointageDate").value = today;
  document.getElementById("pointageDate").disabled = true;
  document.getElementById("pointageEntree").value = existing
    ? existing.entree
    : "08:00";
  document.getElementById("pointageSortie").value = existing
    ? existing.sortie
    : "16:00";
  document.getElementById("pointageStatut").value = existing
    ? existing.statut
    : "Présent";
  document.getElementById("pointageNotes").value = existing
    ? existing.notes || ""
    : "";
  openModal("pointageModal");
}
...
async function enregistrerPointage() {
  ...
  if (empIndex < 0 || empIndex >= employees.length) {
    showToast("⚠️ Employé invalide", "error");
    return;
  }
  const e = employees[empIndex];
  const nomComplet = `${e.prenom} ${e.nom}`.trim();
  const date =
    document.getElementById("pointageDate").value || getTodayDateString();
  ...
  const existingIndex = presencesData.findIndex(
    (p) =>
      (p.employe.trim().toLowerCase() === nomComplet.toLowerCase() ||
        (employeeId && p._employeeId === employeeId)) &&
      p.date === date,
  );
  ...
  if (employeeId) {
    try {
      ...
      data.entree = saved.check_in_time
        ? String(saved.check_in_time).slice(0, 5)
        : entree;
      data.sortie = saved.check_out_time
        ? String(saved.check_out_time).slice(0, 5)
        : sortie;
      data.statut = _mapAttendanceStatusFromAPI(saved.status);
    } catch (err) {
      console.error("[RH] enregistrerPointage API échoué:", err);
      showToast(
        "❌ La présence n’a pas été enregistrée dans la base de données.",
        "error",
      );
      return;
    }
    ...
    showToast(
      "❌ Fiche employé introuvable : présence non enregistrée.",
      "error",
    );
    return;
  }
  ...
  if (existingIndex >= 0) {
    presencesData[existingIndex] = data;
  } else {
    presencesData.unshift(data);
  }
...
function renderConges() {
  ...
  let rows = congesData
    .map(
      (c) => `<tr><td class="emp-name">${c.employe}</td>
                ...
                <button class="btn btn-sm btn-outline btn-icon" onclick="voirHistConge('${c.employe}')"><i class="fas fa-history"></i></button></div></td></tr>`,
    )
    .join("");
  return `<div class="stats-grid">
...
function openCongeModal() {
  ...
  document.getElementById("congeEmp").innerHTML = persistedEmployees
    .map(
      (e) => `<option value="${e._hrEmployeeId}">${e.prenom} ${e.nom}</option>`,
    )
    .join("");
  document.getElementById("congeType").innerHTML = leaveTypesFromAPI
    .map((lt) => `<option value="${lt.id}">${lt.name}</option>`)
    .join("");
...
async function soumettreConge() {
  ...
  if (!employeeId || !debut || !fin) {
    showToast("⚠️ Champs requis", "error");
    return;
  }
  ...
  const leaveType = leaveTypesFromAPI.find(
    (lt) => String(lt.id) === String(leaveTypeId),
  );
  ...
  if (employeeId && leaveType) {
    try {
      ...
      showToast(
        `✅ Demande soumise (${jours} jours) — Enregistrée dans la base`,
        "success",
      );
      return;
    } catch (err) {
  ...
  showToast(
    "❌ Cet employé ou type de congé n’est pas synchronisé avec la base",
    "error",
  );
}
...
async function approuverConge(id) {
  ...
  if (c._apiId) {
    try {
      ...
      console.warn("[RH] approveLeave échoué:", err);
    }
  }
  c.statut = "Approuvé";
  document.getElementById("mainContent").innerHTML = renderConges();
...
async function refuserConge(id) {
  ...
  if (c._apiId) {
    try {
      ...
      console.warn("[RH] rejectLeave échoué:", err);
    }
  }
  c.statut = "Refusé";
  document.getElementById("mainContent").innerHTML = renderConges();
...
function voirHistConge(employe) {
  document.getElementById("histCongeTitle").innerHTML =
    `<i class="fas fa-history"></i> Historique Congés - ${employe}`;
  const hist = congesData.filter((c) => c.employe === employe);
  let html = hist.length
    ? `<div class="hist-cards">` +
      hist
        .map((c) => {
          const iconColor = c.type.includes("maladie")
            ? "var(--red)"
            : c.type.includes("annuel")
              ? "var(--info)"
              : "var(--warning)";
          const iconBg = c.type.includes("maladie")
            ? "var(--red-light)"
            : c.type.includes("annuel")
              ? "var(--info-light)"
              : "var(--warning-light)";
        ...
        .join("") +
      `</div>`
    : '<div class="alert-info"><i class="fas fa-info-circle"></i> Aucun historique</div>';
...
function renderSalaires() {
  ...
  if (salaireFromAPI.length > 0) {
    ...
    rows = employees
      .map((e) => {
        const base = e.salaire || 50000,
          prime = e.prime || 5000,
          ded = Math.round(base * 0.08),
          net = base + prime - ded;
        totalBrut += base + prime;
        totalNet += net;
...
function genererFicheAPI(salaryId) {
  ...
  const empLocal = {
    prenom: empName?.split(" ")[0] || "Employé",
    nom: empName?.split(" ").slice(1).join(" ") || "",
    id: "S-" + s.id,
    salaire: parseFloat(s.base_salary) || 0,
    prime: parseFloat(s.bonuses) || 0,
  };
...
async function marquerPaye(salaryId) {
  try {
    await HRAPI.markSalaryPaid(salaryId, {
...
function calculerPayroll() {
  showToast("✅ Payroll calculé", "success");
  loadAndRenderSalaires();
}
...
function exportSalairesPDF() {
  try {
    ...
    doc.setFontSize(10);
    doc.text(
      `Rapport des Salaires — ${new Date().toLocaleDateString("fr-FR")}`,
      pageWidth - 14,
      14,
      { align: "right" },
    );
    ...
    const body = employees.map((e) => {
      ...
      return [
        ...
        `${e.prenom} ${e.nom}`,
        e.fonction,
        `${base.toLocaleString()} HTG`,
        `${prime.toLocaleString()} HTG`,
        `${ded.toLocaleString()} HTG`,
        `${net.toLocaleString()} HTG`,
      ];
    });
    ...
    doc.autoTable({
      ...
      foot: [
        [
          ...
          "",
          "TOTAL",
          `${totalBase.toLocaleString()} HTG`,
          `${totalPrime.toLocaleString()} HTG`,
          `${totalDed.toLocaleString()} HTG`,
...
function voirHistSalaire(i) {
  const e = employees[i];
  document.getElementById("histSalaireTitle").innerHTML =
    `<i class="fas fa-history"></i> Salaire - ${e.prenom} ${e.nom}`;
...
function renderFichePaie() {
  let rows = employees
    .map(
      (e) =>
        `<tr><td><div class="emp-name">${e.prenom} ${e.nom}</div><div class="emp-detail">${e.id}</div></td><td>${e.fonction}</td><td>${e.dept}</td><td style="font-weight:800;color:var(--blue)">${((e.salaire || 50000) + (e.prime || 5000) - Math.round((e.salaire || 50000) * 0.08)).toLocaleString()} HTG</td><td><button class="btn btn-sm btn-primary" onclick="genererFiche('${e.id}')"><i class="fas fa-receipt"></i> Voir Fiche</button></td></tr>`,
    )
    .join("");
...
function showFichePaieModal(emp) {
  const base = emp.salaire || 50000,
    prime = emp.prime || 5000;
  const taks = Math.round(base * 0.15),
    ofatma = Math.round(base * 0.06),
    dedTotal = taks + ofatma,
    net = base + prime - dedTotal;
...
function renderRecrutement() {
  const source = _apiReady
    ? `<span style="font-size:.75rem;color:var(--success);margin-left:8px"><i class="fas fa-circle" style="font-size:.5rem"></i> API</span>`
    : `<span style="font-size:.75rem;color:var(--warning);margin-left:8px"><i class="fas fa-circle" style="font-size:.5rem"></i> Local</span>`;
  let rows = candidatsData
    .map(
      (
        c,
        i,
      ) => `<tr><td><div class="emp-name">${c.prenom} ${c.nom}</div><div class="emp-detail">${c.dateCandidature}</div></td><td>${c.poste}</td><td>${c.tel}</td><td>${c.email}</td>
...
function voirCandidat(i) {
  ...
  if (!c) return;
  document.getElementById("detailsCandidatTitle").innerHTML =
    `<i class="fas fa-eye"></i> ${c.prenom} ${c.nom}`;
  document.getElementById("detailsCandidatContent").innerHTML =
    `<div class="detail-grid">
                ...
                <div class="detail-item" style="grid-column:1/-1"><div class="d-label">Documents du postulant</div><div class="d-value">${
                  [
                    c.cvFileUrl
                      ? `<a href="${c.cvFileUrl}" target="_blank" rel="noopener"><i class="fas fa-file"></i> ${c.cvFileName || "CV"}</a>`
                      : "",
                    ...(c.documents || []).map(
                      (doc) =>
                        `<a href="${doc.file}" target="_blank" rel="noopener" style="margin-left:12px"><i class="fas fa-paperclip"></i> ${doc.filename}</a>`,
                    ),
                  ]
                    .filter(Boolean)
                    .join("") || "Aucun document"
...
function editCandidat(i) {
  ...
  const c = candidatsData[i];
  document.getElementById("modalCandidatTitle").innerHTML =
    `<i class="fas fa-edit"></i> Modifier - ${c.prenom} ${c.nom}`;
  ...
  document.getElementById("candDate").value = c.dateCandidature
    .split("/")
    .reverse()
    .join("-");
...
async function saveCandidat() {
  ...
  if (!p || !n) {
    showToast("⚠️ Prénom et nom requis", "error");
    return;
  }
  const applicationDate =
    document.getElementById("candDate").value || getTodayDateString();
  const existing =
    editingCandidatIndex !== null ? candidatsData[editingCandidatIndex] : null;
  ...
  if (existing?.statut)
    formData.append("status", _mapCandidateStatusToAPI(existing.statut));
  ...
  try {
    ...
    console.error(
      "[RH] Enregistrement du candidat ou de ses documents échoué:",
      err,
    );
    showToast(
      "❌ Enregistrement impossible : aucun document n’a été conservé localement.",
      "error",
    );
  }
}

async function _updateCandidateStatus(
  index,
  statusApi,
  toastMsg,
  toastType = "success",
...
function programmerEntretien(i) {
  const c = candidatsData[i];
  if (!c) return;
  document.getElementById("entretienTitle").innerHTML =
    `<i class="fas fa-calendar-check"></i> Programmer Entretien`;
  document.getElementById("entretienCandidatIndex").value = i;
  document.getElementById("entretienCandidatNom").value =
  ...
  document.getElementById("entretienInterviewer").innerHTML =
    `<option selected>${admin.first_name || ""} ${admin.last_name || admin.email || "Administrateur connecté"}</option>`;
...
async function confirmerEntretien() {
  ...
  const c = candidatsData[i];
  if (!c?._apiId) {
    showToast("⚠️ Candidat non synchronisé", "error");
    return;
  }
  try {
    await HRAPI.scheduleInterview(c._apiId, {
      interview_date: date,
      interview_time: heure,
      notes,
    });
...
async function accepterCandidat(i) {
  ...
  if (!candidate) return;
  if (
    confirm(`✅ Confirmer ${candidate.prenom} ${candidate.nom} comme employé ?`)
  ) {
    try {
      if (candidate._apiId) {
      ...
      updateBadges();
      showToast(
        `✅ ${candidate.prenom} ${candidate.nom} est maintenant employé`,
        "success",
      );
    } catch (err) {
      showToast(
...
async function loadAndRenderDocuments() {
  ...
  try {
        ...
        ? [
            {
              ...
              nom: String(candidate.cv_file).split("/").pop(),
              employe: owner,
              type: "CV",
              date: candidate.created_at
                ...
                : "—",
              taille: "—",
              fileUrl: candidate.cv_file,
              source: "candidate-cv",
            },
          ]
        : [];
      const attachments = (candidate.documents || []).map((doc) => ({
        _candidateDocumentId: doc.id,
        _candidateId: candidate.id,
        nom: doc.filename || `doc_${doc.id}`,
        employe: owner,
        type: _mapDocType(doc.document_type),
        date: doc.created_at
    ...
    _apiReady = false;
    _apiErrorMessage =
      "Les documents ne sont pas disponibles : connexion à la base impossible.";
  }
  document.getElementById("mainContent").innerHTML = renderDocuments();
}
...
function telechargerDocAPI(idx) {
  ...
  if (d.fileUrl) {
    const a = document.createElement("a");
    a.href = d.fileUrl.startsWith("http")
      ? d.fileUrl
      : API_CONFIG.BASE_URL.replace("/api/v1", "") + d.fileUrl;
    a.download = d.nom;
    a.target = "_blank";
    a.click();
...
async function loadAndRenderEvaluations() {
  ...
  try {
    ...
    _apiReady = false;
    _apiErrorMessage =
      "Les évaluations ne sont pas disponibles : connexion à la base impossible.";
  }
...
function renderEvaluations() {
  const source = _apiReady
    ? `<span style="font-size:.75rem;color:var(--success);margin-left:8px"><i class="fas fa-circle" style="font-size:.5rem"></i> API</span>`
    : `<span style="font-size:.75rem;color:var(--danger);margin-left:8px"><i class="fas fa-circle" style="font-size:.5rem"></i> Indisponible</span>`;
  let rows = evaluationsData
    .map(
      (ev, i) => `<tr><td class="emp-name">${ev.employe}</td><td>${ev.date}</td>
...
function openEvalModal() {
  ...
  document.getElementById("evalEmp").innerHTML = employees
    .filter((e) => e._hrEmployeeId)
    .map((e) => `<option>${e.prenom} ${e.nom}</option>`)
    .join("");
...
function editEvaluation(i) {
  ...
  document.getElementById("evalEmp").innerHTML = employees
    ...
    .map(
      (e) =>
        `<option ${e.prenom + " " + e.nom === ev.employe ? "selected" : ""}>${e.prenom} ${e.nom}</option>`,
    )
    .join("");
...
function deleteEvaluation(i) {
  ...
  if (confirm(`⚠️ Supprimer l'évaluation de ${ev.employe} ?`)) {
    if (!ev._apiId) {
      showToast(
        "❌ Évaluation non synchronisée : suppression refusée",
        "error",
      );
      return;
    }
    HRAPI.deleteEvaluation(ev._apiId)
      ...
      .then(() =>
        showToast(`🗑️ Évaluation de ${ev.employe} supprimée`, "success"),
      )
      .catch((err) =>
        showToast(
...
async function saveEvaluation() {
  ...
  const existing =
    editingEvalIndex !== null ? evaluationsData[editingEvalIndex] : null;
  ...
  if (employeeId) {
    try {
      ...
      if (existing?._apiId) {
        ...
        showToast(
          `✅ Évaluation de ${emp} modifiée — Score: ${score}%`,
          "success",
        );
      } else {
...
function exportEmployesCSV() {
  ...
  employees.forEach(
    (e) =>
      (csv += `${e.id},${e.nom},${e.prenom},${e.fonction},${e.dept},${e.statut},${e.salaire || 0}\n`),
  );
  const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
...
function exportEmployesPDF() {
  try {
    ...
    doc.setFontSize(10);
    doc.text(
      `Liste des Employés — ${new Date().toLocaleDateString("fr-FR")}`,
      pageWidth - 14,
      14,
      { align: "right" },
    );
    doc.setFontSize(8);
    doc.text(`${employees.length} employé(s)`, pageWidth - 14, 19, {
      align: "right",
    });
...
function exportProfilPDF() {
  try {
    if (selectedProfilIndex === null) {
      showToast("⚠️ Aucun profil sélectionné", "error");
      return;
    }
    ...
    if (e.cours && e.cours.length) {
      let y =
        ...
        10 +
        (e.diplomes?.length ? e.diplomes.length * 6 + 12 : 0);
      doc.setFontSize(11);
      doc.setFont(undefined, "bold");
      doc.setTextColor(10, 77, 140);
    ...
    doc.setTextColor(139, 149, 165);
    doc.text(
      `Généré le ${new Date().toLocaleDateString("fr-FR")} — Module RH CEJEC`,
      14,
...
function exportFichePDF() {
  try {
    if (!selectedFicheEmp) {
      showToast("⚠️ Aucune fiche sélectionnée", "error");
      return;
    }
    ...
    doc.setFontSize(11);
    doc.text(
      `Fiche de Paie — ${new Date().toLocaleDateString("fr-FR", { month: "long", year: "numeric" })}`,
      pageWidth - 14,
      19,
      { align: "right" },
    );
    ...
    doc.setFontSize(14);
    doc.text(`${net.toLocaleString()}.00 HTG`, pageWidth - 20, finalY + 10, {
      align: "right",
    });
  ...
  employesAujourdhui.forEach((e) => {
    ...
    const p = presencesData.find(
      (att) =>
        ...
        ((e._hrEmployeeId && att._employeeId === e._hrEmployeeId) ||
          (att.employe &&
            att.employe.trim().toLowerCase() === nomComplet.toLowerCase())),
    );
...
function openModal(id) {
  ...
  if (id === "addEmpModal" && editingId === null) {
    ...
    document
      .querySelectorAll("#addEmpModal input, #addEmpModal select")
      .forEach((el) => {
        if (
          el.type !== "hidden" &&
          el.id !== "empStatut" &&
  ...
  if (id === "addCandidatModal" && editingCandidatIndex === null) {
    document.getElementById("modalCandidatTitle").innerHTML =
      '<i class="fas fa-briefcase"></i> Nouveau Candidat';
  ...
  if (id === "uploadDocModal") {
    document.getElementById("docEmp").innerHTML = employees
      .filter((e) => e._hrEmployeeId)
      .map((e) => `<option>${e.prenom} ${e.nom}</option>`)
      .join("");
    const zone = document.getElementById("docUploadZone");
...
async function syncAllEmployees() {
  ...
  if (teachersFromAPI.length > 0) {
    apiProfs = teachersFromAPI.map((t) => {
      const usr = t.user || {};
      const specialty = t.primary_specialty
        ? [t.primary_specialty.subject]
        : [];
      const hrMatch =
        staffByEmail[(usr.email || "").toLowerCase()] ||
        apiStaff.find(
          (s) => s.fonction === "Professeur" && s.id === t.teacher_id,
        );
      ...
      return {
        ...
        nom: usr.last_name || "",
        sexe:
          usr.profile?.gender === "M"
            ? "Homme"
            : usr.profile?.gender === "F"
              ? "Femme"
              : "N/A",
        tel: usr.phone || "N/A",
        email: usr.email || "N/A",
  ...
  const profHrIds = new Set(
    apiProfs.map((p) => p._hrEmployeeId).filter(Boolean),
  );
  const staffOnly = apiStaff.filter(
    (s) => s.fonction !== "Professeur" && !profHrIds.has(s._hrEmployeeId),
  );
  ...
  const knownNames = new Set(
    [...staffOnly, ...apiProfs].map(
      (e) =>
        `${(e.prenom || "").trim().toLowerCase()} ${(e.nom || "").trim().toLowerCase()}`,
    ),
  );
  ...
  const recruitedEmployees = (candidatsData || [])
    ...
    .map((c, idx) => ({
      ...
      embauche: c.dateCandidature
        ? c.dateCandidature.split("/").reverse().join("-")
        : getTodayDateString(),
...
async function deleteEmp(i) {
  ...
  if (
    !employee ||
    !confirm(`⚠️ Supprimer ${employee.prenom} ${employee.nom} ?`)
  )
    return;
  try {
    ...
    if (employee._hrEmployeeId) {
      ...
      console.warn(
        "[RH] Delete operation skipped for non‑HR employee",
        employee,
      );
    }
...
async function saveEmployee() {
  ...
  const isProfesseur =
    document.getElementById("empFonction").value === "Professeur";
  const statuses = {
    Actif: "active",
    Suspendu: "suspended",
    Congé: "inactive",
  ...
  try {
    const body = {
      employee_number:
        ...
        `${matriculePrefix}${Date.now().toString().slice(-6)}`,
      first_name: prenom,
      last_name: nom,
    ...
    if (current?._hrEmployeeId)
      await HRAPI.updateEmployee(current._hrEmployeeId, body);
    ...
    showToast(
      "❌ Enregistrement impossible. Vérifiez la connexion et vos droits.",
      "error",
    );
  }
}
    ...
    if (select && academicYearsRH.length > 0) {
      select.innerHTML =
        ...
        academicYearsRH
          .map(
            (y) =>
              `<option value="${y.label}" ${currentAcademicYearRH === y.label ? "selected" : ""}>${y.label}${y.is_active ? " (Active)" : ""}</option>`,
          )
          .join("");
    }
  } catch (e) {
...
async function initRH() {
  ...
  _apiReady = Boolean(
    teachersFromAPI.length ||
    employees.some((e) => e._hrEmployeeId) ||
    leaveTypesFromAPI.length,
  );
  ...
  if (targetTeacherId) {
    ...
    const idx = employees.findIndex(
      (e) =>
        String(e._teacherId) === String(targetTeacherId) ||
        String(e.id) === String(targetTeacherId),
    );
    if (idx !== -1) {
    ...
    const idx = employees.findIndex(
      (e) =>
        String(e._hrEmployeeId) === String(targetEmpId) ||
    ...
    const idx = candidatsData.findIndex(
      (c) => String(c.id) === String(targetCandidatId),
    );
    if (idx !== -1) {
