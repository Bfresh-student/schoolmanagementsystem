...
function getCurrentClassData() {
  return schoolClasses.find((item) => item.name === currentClass);
}
function refreshCurrentCourses() {
  const schoolClass = getCurrentClassData();
  evaluations = assessments
    .filter(
      (item) =>
        (!schoolClass ||
          String(item.school_class) === String(schoolClass.id)) &&
        item.academic_year === currentYear,
    )
    .map((item) => ({
      id: String(item.id),
      matiere: item.course_name,
      type: item.title,
      evaluationType: item.evaluation_type,
      date: item.evaluation_date,
      coef: Number(item.coefficient),
      comment: item.term,
      annee: item.academic_year,
      classe: item.school_class_name,
    }));
  matieres = [
    ...new Set(
      evaluations.map((evaluation) => evaluation.matiere).filter(Boolean),
    ),
  ];
}
async function loadRealData() {
  ...
  schoolClasses = loadedClasses.filter((item) => item.is_active);
  courses = loadedCourses.filter((item) => item.status === "active");
  assessments = loadedAssessments;
  classes = schoolClasses.map((item) => item.name);
  if (classes.length) currentClass = classes[0];
  const apiYears = (Array.isArray(loadedYears) ? loadedYears : []).map(
    (y) => y.label,
  );
  const assessmentYears = assessments
    .map((item) => item.academic_year)
    .filter(Boolean);
  academicyears = [...new Set([...apiYears, ...assessmentYears])];
  const activeYearObj = (Array.isArray(loadedYears) ? loadedYears : []).find(
    (y) => y.is_active,
  );
  currentYear =
    (activeYearObj ? activeYearObj.label : null) ||
    academicyears[0] ||
  ...
  loadedGrades.forEach((grade) => {
    notes[`${grade.student}_${grade.assessment || grade.course}`] = Number(
      grade.value,
    );
  });
}
function getEvaluationForMatiere(reference) {
  return (
    evaluations.find(
      (evaluation) => String(evaluation.id) === String(reference),
    ) || evaluations.find((evaluation) => evaluation.matiere === reference)
  );
}
async function persistNote(etudiantId, matiere, value, date) {
  const evaluation = getEvaluationForMatiere(matiere);
  if (!evaluation) throw new Error("EVALUATION_INTROUVABLE");
  const result = await GradesAPI.submit({
    student: etudiantId,
    assessment: evaluation.id,
    value,
    date,
  });
  if (result.outcome === "conflict") throw new Error("CONFLIT_NOTE");
  notes[`${etudiantId}_${evaluation.id}`] = value;
}
function getRecordedNoteCount(studentId = null) {
  const students =
    studentId == null
      ? getFilteredEtudiants().map((student) => student.id)
      : [studentId];
  return students.reduce(
    (count, id) =>
      ...
      evaluations.filter(
        (evaluation) =>
          notes[`${id}_${evaluation.id}`] !== undefined &&
          notes[`${id}_${evaluation.id}`] !== null,
      ).length,
    0,
  );
}
...
function initSampleData() {
  ...
  matieres.forEach((mat, idx) => {
    ...
    etudiants
      .filter((e) => e.classe === "ENTREPRENEURIAT" && e.annee === "2025-2026")
      .forEach((et) => {
        notes[`${et.id}_${evalId1}`] =
          Math.round((55 + Math.random() * 45) * 2) / 2;
      });
    ...
    evaluations.push({
      ...
      matiere: mat,
      type: "Examen",
      date: "2026-05-" + String(15 + idx).padStart(2, "0"),
      coef: 3,
      comment: "Examen Final",
      annee: "2025-2026",
      classe: "ENTREPRENEURIAT",
    });
    etudiants
      .filter((e) => e.classe === "ENTREPRENEURIAT" && e.annee === "2025-2026")
      .forEach((et) => {
        notes[`${et.id}_${evalId2}`] =
          Math.round((45 + Math.random() * 55) * 2) / 2;
      });
  });
}
...
function getFilteredEtudiants() {
  ...
  let filtered = etudiants.filter(
    (e) => e.annee === currentYear && e.classe === currentClass,
  );
  if (search)
    filtered = filtered.filter((e) => e.nom.toLowerCase().includes(search));
  if (mention !== "tous") {
...
function getMatiereAverage(etudiantId, matiere) {
  ...
  evaluations.forEach((ev) => {
    if (
      ev.matiere === matiere &&
      ev.annee === currentYear &&
      ev.classe === currentClass
    ) {
      const key = `${etudiantId}_${ev.id}`;
      if (
        notes[key] !== undefined &&
  ...
  return totalCoef > 0
    ? Math.round((totalNotes / totalCoef) * 100) / 100
    : null;
}
...
function getClassRank(etudiantId) {
  ...
  const avgs = filtered
    .map((e) => ({ id: e.id, avg: getStudentAverage(e.id) }))
...
function renderTable() {
  ...
  const classEvals = evaluations.filter(
    (ev) => ev.annee === currentYear && ev.classe === currentClass,
  );
  const head = document.getElementById("tableHead");
  const body = document.getElementById("tableBody");

  let headHTML =
    '<tr><th class="corner"><i class="fas fa-user"></i> Élève</th>';
  matieres.forEach((mat) => {
    const count = classEvals.filter((ev) => ev.matiere === mat).length;
    const shortName = mat.length > 22 ? mat.substring(0, 20) + "..." : mat;
    headHTML += `<th class="th-matiere" title="${mat}"><i class="fas fa-book"></i> <span class="th-title">${shortName}</span><br><small style="font-weight:400;font-size:.62rem;opacity:0.85">${count} note(s)</small></th>`;
  });
  headHTML +=
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach((et) => {
      bodyHTML += "<tr>";
      bodyHTML += `<td class="student-cell" onclick="openPanel(${et.id})" title="Détails de l'élève"><div class="student-info-wrap"><span class="avatar-xs" style="background:${getAvatarColor(et.id)}">${getInitials(et.nom)}</span><span class="student-name-text">${et.nom}</span></div></td>`;

      matieres.forEach((mat) => {
        const avg = getMatiereAverage(et.id, mat);
        const noteClass = getNoteClass(avg);
        bodyHTML += `<td class="note-cell" onclick="event.stopPropagation();startCellEdit(this, ${et.id}, '${mat.replace(/'/g, "\\'")}')" title="Cliquer pour ajouter/modifier la note de ${et.nom} en ${mat}">
      ...
      bodyHTML += `<td class="mention-cell"><span class="mention-badge ${mention.class}">${avg > 0 ? mention.text : "-"}</span></td>`;
...
async function finishCellEdit(input, etudiantId, matiere) {
  ...
  if (input.value === "" || Number.isNaN(value)) {
    showToast("La suppression d’une note n’est pas encore disponible.", "info");
    renderTable();
    return;
  }
  if (value < 0 || value > 100) {
    showToast("La note doit être comprise entre 0 et 100.", "error");
    renderTable();
    return;
  }
  try {
    ...
    showToast(
      error.message === "CONFLIT_NOTE"
        ? "Conflit détecté : arbitrage administrateur requis."
...
function openAssessmentModal(id = null) {
  ...
  const existing = id
    ? assessments.find((item) => String(item.id) === String(id))
    : null;
  const schoolClass = getCurrentClassData();
  document.getElementById("assessmentModalTitle").textContent = existing
    ? "Modifier l’évaluation"
    : "Nouvelle évaluation";
  document.getElementById("assessmentCourse").innerHTML = courses
    .map(
      (course) =>
        `<option value="${course.id}">${course.code} — ${course.name}</option>`,
    )
    .join("");
  document.getElementById("assessmentCourse").value = existing?.course || "";
  document.getElementById("assessmentTitle").value = existing?.title || "";
  document.getElementById("assessmentType").value =
    existing?.evaluation_type || "assignment";
  document.getElementById("assessmentCoefficient").value =
    existing?.coefficient || 1;
  document.getElementById("assessmentTerm").value = existing?.term || "";
  ...
  document.getElementById("assessmentDate").value =
    existing?.evaluation_date || new Date().toISOString().slice(0, 10);
  if (!schoolClass) {
    showToast("Sélectionnez une classe active.", "error");
    return;
  }
...
async function saveAssessment() {
  const schoolClass = getCurrentClassData();
  const payload = {
    course: document.getElementById("assessmentCourse").value,
    school_class: schoolClass?.id,
    title: document.getElementById("assessmentTitle").value.trim(),
    evaluation_type: document.getElementById("assessmentType").value,
    coefficient: Number(document.getElementById("assessmentCoefficient").value),
    term: document.getElementById("assessmentTerm").value.trim(),
    academic_year: document.getElementById("assessmentYear").value.trim(),
    evaluation_date: document.getElementById("assessmentDate").value,
  };
  if (
    !payload.course ||
    ...
    payload.coefficient <= 0
  ) {
    showToast("Complétez tous les champs requis.", "error");
    return;
  }
  try {
    const saved = editingAssessmentId
      ? await AssessmentsAPI.update(editingAssessmentId, payload)
      : await AssessmentsAPI.create(payload);
    const index = assessments.findIndex(
      (item) => String(item.id) === String(saved.id),
    );
    if (index >= 0) assessments[index] = saved;
    else assessments.push(saved);
    if (!academicyears.includes(saved.academic_year))
      academicyears.push(saved.academic_year);
    currentYear = saved.academic_year;
    updateYearFilter();
    refreshCurrentCourses();
    ...
    showToast("Enregistrement de l’évaluation impossible.", "error");
  }
}
...
function openAddNoteModal() {
  ...
  studentSelect.innerHTML =
    ...
    etudiants
      .filter((e) => e.annee === currentYear && e.classe === currentClass)
  ...
  matiereSelect.innerHTML =
    ...
    evaluations
      .map(
        (e) =>
          `<option value="${e.id}">${e.matiere} · ${e.type} · coef ${e.coef}</option>`,
      )
      .join("");
  matiereSelect.onchange = () => {
    const e = getEvaluationForMatiere(matiereSelect.value);
    const labels = {
      quiz: "Quiz",
      assignment: "Devoir",
      exam: "Examen",
    ...
    document.getElementById("addNoteCoef").value = e?.coef || "";
    document.getElementById("addNoteType").value =
      labels[e?.evaluationType] || "";
  };

  document.getElementById("addNoteDate").value = new Date()
    .toISOString()
    .split("T")[0];
...
async function saveNewNote() {
  const etudiantId = Number(document.getElementById("addNoteStudent").value);
  const matiere = document.getElementById("addNoteMatiere").value;
  const noteVal = Number(document.getElementById("addNoteValue").value);
  const date = document.getElementById("addNoteDate").value;
  if (
    !etudiantId ||
    !matiere ||
    Number.isNaN(noteVal) ||
    noteVal < 0 ||
    noteVal > 100
  ) {
    showToast(
      "Sélectionnez un élève, un cours et une note entre 0 et 100.",
      "error",
    );
    return;
  }
  try {
    await persistNote(etudiantId, matiere, noteVal, date);
    closeAddNoteModal();
    renderTable();
    showToast("Note enregistrée.", "success");
  } catch (error) {
    showToast(
      error.message === "CONFLIT_NOTE"
        ? "Conflit détecté : la note doit être arbitrée par un administrateur."
        : "Enregistrement impossible.",
      "error",
    );
  }
...
function updateStats() {
  ...
  const success = avgs.filter((a) => a >= PASSING_GRADE).length;
  document.getElementById("statSuccess").textContent =
    avgs.length > 0 ? Math.round((success / avgs.length) * 100) + "%" : "0%";
  const low = avgs.filter((a) => a < PASSING_GRADE).length;
...
function updateCharts() {
  ...
  if (ctx1) {
    ...
    const matAvgs = matieres.map((mat) => {
      ...
      filtered.forEach((et) => {
        const avg = getMatiereAverage(et.id, mat);
        if (avg !== null) allNotes.push(avg);
      });
      return allNotes.length > 0
        ? Math.round(
            (allNotes.reduce((a, b) => a + b, 0) / allNotes.length) * 10,
          ) / 10
        : 0;
    });
    const shortLabels = matieres.map((m) =>
  ...
  if (ctx2) {
    ...
    filtered.forEach((et) => {
      const avg = getStudentAverage(et.id);
      if (avg > 0)
        mentionsCount[getMention(avg).text] =
          (mentionsCount[getMention(avg).text] || 0) + 1;
    });
    const colors = [
      "#10b981",
      "#3b82f6",
    ...
    chartInstances["pie"] = new Chart(ctx2, {
      type: "doughnut",
      data: {
        labels: Object.keys(mentionsCount),
        datasets: [
          {
            data: Object.values(mentionsCount),
...
function openPanel(etudiantId) {
  ...
  document.getElementById("panelAvatar").textContent = getInitials(
    currentPanelStudent.nom,
  );
  document.getElementById("panelAvatar").style.background =
    getAvatarColor(etudiantId);
  document.getElementById("panelName").textContent = currentPanelStudent.nom;
  document.getElementById("panelMatricule").textContent =
    "Matricule: " + currentPanelStudent.matricule;
...
function panelView() {
  if (currentPanelStudent)
    showToast("Détails de " + currentPanelStudent.nom, "info");
}

function panelEditNotes() {
  if (currentPanelStudent) {
    closePanel();
...
function panelBulletin() {
  if (currentPanelStudent) {
    generateBulletin(currentPanelStudent.id);
    closePanel();
  }
}
...
function panelStats() {
  if (currentPanelStudent) {
    document.getElementById("filterSearch").value = currentPanelStudent.nom;
    applyFilters();
    closePanel();
  }
}
...
function panelPrint() {
  if (currentPanelStudent) {
    printStudent(currentPanelStudent.id);
    closePanel();
  }
}
...
function panelSendEmail() {
  if (currentPanelStudent) {
    showToast("Email envoyé à " + currentPanelStudent.nom, "success");
    closePanel();
  }
}
...
function panelArchive() {
  if (currentPanelStudent) {
    archiveStudent(currentPanelStudent.id);
    closePanel();
  }
}
...
function openEditNotesModal(etudiantId) {
  ...
  matieres.forEach((mat) => {
    ...
    if (matEvals.length > 0) {
      ...
      html += `<h4 style="margin-top:10px;color:var(--blue-dark);font-size:0.82rem"><i class="fas fa-book"></i> ${shortMat}</h4>`;
      matEvals.forEach((ev) => {
        const key = `${etudiantId}_${ev.id}`;
        const val =
          notes[key] !== undefined && notes[key] !== null ? notes[key] : "";
        html += `
...
function closeEditNotesModal() {
  document.getElementById("editNotesModal").classList.remove("open");
  currentEditStudent = null;
}

async function saveEditedNotes() {
  if (!currentEditStudent) return;
  const student = currentEditStudent;
  try {
    for (const input of document.querySelectorAll(".note-edit-input")) {
      if (input.value === "") continue;
      const value = Number(input.value);
      const evaluation = evaluations.find(
        (item) => String(item.id) === input.dataset.eval,
      );
      if (!evaluation || Number.isNaN(value) || value < 0 || value > 100)
    ...
    closeEditNotesModal();
    renderTable();
    showToast(`Notes de ${student.nom} enregistrées.`, "success");
  } catch (error) {
    showToast(
      error.message === "CONFLIT_NOTE"
        ? "Conflit détecté : arbitrage administrateur requis."
        : "Une note est invalide ou n’a pas pu être enregistrée.",
      "error",
    );
  }
}
  ...
  win.document.write(`<html><head><title>Bulletin - ${et.nom} | CEJEC</title>
            ...
            ${matieres
              .map((mat) => {
                const mAvg = getMatiereAverage(etudiantId, mat);
                const m =
                  mAvg !== null ? getMention(mAvg) : { text: "-", class: "" };
                const totalCoef = classEvals
                  .filter((ev) => ev.matiere === mat)
                  .reduce((s, ev) => s + ev.coef, 0);
                return `<tr><td>${mat}</td><td style="text-align:center">${totalCoef}</td><td style="text-align:center;font-weight:700">${mAvg !== null ? mAvg.toFixed(1) : "-"}/100</td><td>${m.text}</td></tr>`;
              })
              .join("")}
            </tbody></table>
...
function generateAllBulletins() {
  const filtered = getFilteredEtudiants();
  if (filtered.length === 0) {
    showToast("Aucun élève", "error");
    return;
  }
  showToast("Génération de " + filtered.length + " bulletins...", "info");
  filtered.forEach((et, i) =>
    setTimeout(() => generateBulletin(et.id), i * 700),
  );
}

function printStudent(etudiantId) {
  generateBulletin(etudiantId);
}
...
function exportToCSV() {
  ...
  matieres.forEach((mat) => {
    const matNotes = [];
    filtered.forEach((et) => {
      const mAvg = getMatiereAverage(et.id, mat);
      if (mAvg !== null) matNotes.push(mAvg);
    });
...
function openYearModal() {
  document.getElementById("existingYears").innerHTML = academicyears
    .map(
      (y) =>
...
function addYear() {
  const input = document.getElementById("newYearInput").value.trim();
  if (input && !academicyears.includes(input)) {
    academicyears.push(input);
    updateYearFilter();
    closeYearModal();
    showToast("Année " + input + " ajoutée", "success");
  } else if (input) {
    showToast("Cette année existe déjà", "error");
  }
}
...
function updateYearFilter() {
  document.getElementById("filterYear").innerHTML = academicyears
    .map(
      (y) =>
...
function openClassModal() {
  document.getElementById("existingClasses").innerHTML = classes
    .map(
      (c) =>
...
function addClass() {
  const input = document
    .getElementById("newClassInput")
    .value.trim()
    .toUpperCase();
  if (input && !classes.includes(input)) {
    classes.push(input);
    updateClassFilter();
    closeClassModal();
    showToast("Classe " + input + " ajoutée", "success");
  } else if (input) {
...
function updateClassFilter() {
  document.getElementById("filterClass").innerHTML = classes
    .map(
      (c) =>
...
function applyFilters() {
  ...
  if (yearVal && yearVal !== currentYear) currentYear = yearVal;
  if (classVal && classVal !== currentClass) {
...
addYear = () =>
  showToast(
    "Les années académiques ne sont pas encore gérées par le backend.",
    "info",
  );
addClass = () =>
  showToast("Créez une classe depuis la gestion des classes.", "info");
archiveStudent = () =>
  showToast(
    "L’archivage doit être effectué depuis la fiche de l’élève.",
    "info",
  );
async function init() {
  try {
    ...
    console.error("Chargement des notes impossible", error);
    etudiants = [];
    evaluations = [];
    matieres = [];
    showToast("Impossible de charger les données réelles.", "error");
  }
  updateYearFilter();
  updateClassFilter();
  renderTable();
