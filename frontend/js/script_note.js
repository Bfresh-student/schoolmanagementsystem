        ...
        async function loadRealData() { const [students, loadedClasses, loadedCourses, loadedGrades, loadedAssessments, loadedYears] = await Promise.all([StudentsAPI.list(), ClassesAPI.list(), CoursesAPI.list(), GradesAPI.list(), AssessmentsAPI.list(), typeof AcademicYearsAPI !== 'undefined' ? AcademicYearsAPI.list() : Promise.resolve([])]); schoolClasses = loadedClasses.filter(item => item.is_active); courses = loadedCourses.filter(item => item.status === 'active'); assessments = loadedAssessments; classes = schoolClasses.map(item => item.name); if (classes.length) currentClass = classes[0]; const apiYears = (Array.isArray(loadedYears) ? loadedYears : []).map(y => y.label); const assessmentYears = assessments.map(item => item.academic_year).filter(Boolean); academicyears = [...new Set([...apiYears, ...assessmentYears])]; const activeYearObj = (Array.isArray(loadedYears) ? loadedYears : []).find(y => y.is_active); currentYear = (activeYearObj ? activeYearObj.label : null) || academicyears[0] || `${new Date().getFullYear()}-${new Date().getFullYear() + 1}`; etudiants = students.map(student => ({ id: student.id, nom: student.full_name || `Étudiant #${student.id}`, matricule: student.registration_number || `ETU-${student.id}`, telephone: '', classe: student.school_class_name || 'Sans classe', annee: currentYear })); refreshCurrentCourses(); notes = {}; loadedGrades.forEach(grade => { notes[`${grade.student}_${grade.assessment || grade.course}`] = Number(grade.value); }); }
