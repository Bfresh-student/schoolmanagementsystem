...
dom.window.eval('window.showToast = function(){}; window.localStorage = {getItem:()=>null}; window.getAccessToken=()=>null; window.pdfjsLib = { GlobalWorkerOptions: {} }; window.jspdf = { jsPDF: function(){} };');
const script = fs.readFileSync('frontend/js/api-client.js', 'utf8') + '\n' + fs.readFileSync('frontend/js/script_rh.js', 'utf8');
try {
  dom.window.eval(script);
  console.log('Script ran successfully');
} catch (e) {
  console.error('ERROR IN SCRIPT:', e.stack);
}
